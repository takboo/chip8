  ...
  async function setupDragDrop() {
    ...
    const unlisten = await webview.onDragDropEvent((event) => {
      ...
      if (event.payload.type === "enter") {
        console.log("User entering drop zone with files:", event.payload.paths);
        isDragOver = true;
  ...
  <div class="main-content">
    <div class="controls-panel">
      ...
      <div class="options">
        ...
        <label class="checkbox-container">
          <input type="checkbox" bind:checked={showCrtEffect} />
          <span class="checkmark"></span>
          CRT EFFECTS
        </label>
    ...
    <div class="display-container">
      <div
        class="drop-zone {isDragOver ? 'drag-over' : ''} {showCrtEffect
          ? 'crt-active'
          : ''}"
        role="application"
    ...
    >
      ...
      >
        <div class="help-header">
          ...
          <button class="close-btn" onclick={() => (showKeyboardHelp = false)}
            >×</button
          >
        </div>
        ...
        <div class="help-content">
          ...
          <div class="help-shortcuts">
            ...
            <ul>
              <li>
                <kbd>F1</kbd> / <kbd>H</kbd>: SHOW/HIDE KEYBOARD MAPPING
              </li>
            </ul>
          </div>
        </div>
...
>
  <defs>
    <clipPath id="crtPath" clipPathUnits="objectBoundingBox">
      <path
        d="M 0.01,0.01 C 0.3,0, 0.7,0, 0.99,0.01 C 1,0.3, 1,0.7, 0.99,0.99 C 0.7,1, 0.3,1, 0.01,0.99 C 0,0.7, 0,0.3, 0.01,0.01 Z"
      />
    </clipPath>
  </defs>
</svg>
...
<style>
  ...
  .drop-zone.crt-active {
    clip-path: url(#crtPath);
  }

  .drop-zone.crt-active::before,
  .drop-zone.crt-active::after {
    content: "";
    display: block;
    position: absolute;
    pointer-events: none;
  ...
  .drop-zone.crt-active::after {
    ...
    z-index: 5;
    animation: scan-crt 0.4s steps(1, start) infinite;
  }
  ...
  .drop-zone.crt-active::before {
    ...
    width: 100%;
    height: 0.2rem;
    background: rgba(26, 255, 118, 0.15);
    opacity: 0.75;
    z-index: 6;
